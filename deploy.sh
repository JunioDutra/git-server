...
NON_SECRET_ENV_NAMES="GIT_HTTP_HOST GIT_HTTP_PORT GIT_SSH_HOST GIT_DEFAULT_BRANCH GIT_REPOS_ROOT GIT_HOOKS_ROOT GIT_HOOK_LOGS_ROOT GIT_HOOK_LOG_RETENTION_DAYS GIT_BUILD_SOCKET"
...
for file in post-receive mirror-sync.sh build_image.py build_submit.py build_dispatcher.py; do
  ssh -o StrictHostKeyChecking=accept-new -o ConnectTimeout=10 "$PROX_TARGET" \
...
ssh -o StrictHostKeyChecking=accept-new -o ConnectTimeout=10 "$PROX_TARGET" \
  "pct exec $GIT_CONTAINER_ID -- python3 -m py_compile $CT_DIR/app.py.new $CT_DIR/configure_build_env.py.new /home/git/hooks/build_image.py.new /home/git/hooks/build_submit.py.new /home/git/hooks/build_dispatcher.py.new" 2>&1
ssh -o StrictHostKeyChecking=accept-new -o ConnectTimeout=10 "$PROX_TARGET" \
...
ssh -o StrictHostKeyChecking=accept-new -o ConnectTimeout=10 "$PROX_TARGET" \
  "pct exec $GIT_CONTAINER_ID -- sh -c 'set -e; stamp=\$(date -u +%Y%m%dT%H%M%SZ); for file in app.py configure_build_env.py; do [ ! -f $CT_DIR/\$file ] || cp -p $CT_DIR/\$file $CT_DIR/\$file.bak-\$stamp; done; for service in git-http-server git-build-dispatcher; do [ ! -f /etc/init.d/\$service ] || cp -p /etc/init.d/\$service /etc/init.d/\$service.bak-\$stamp; done; for file in post-receive mirror-sync.sh build_image.py build_submit.py build_dispatcher.py; do [ ! -f /home/git/hooks/\$file ] || cp -p /home/git/hooks/\$file /home/git/hooks/\$file.bak-\$stamp; done; mv $CT_DIR/app.py.new $CT_DIR/app.py; mv $CT_DIR/configure_build_env.py.new $CT_DIR/configure_build_env.py; mv /etc/init.d/git-http-server.new /etc/init.d/git-http-server; mv /etc/init.d/git-build-dispatcher.new /etc/init.d/git-build-dispatcher; for file in post-receive mirror-sync.sh build_image.py build_submit.py build_dispatcher.py; do mv /home/git/hooks/\$file.new /home/git/hooks/\$file; done; chown git:git $CT_DIR/app.py /home/git/hooks/post-receive /home/git/hooks/mirror-sync.sh /home/git/hooks/build_image.py /home/git/hooks/build_submit.py /home/git/hooks/build_dispatcher.py; chown root:root $CT_DIR/configure_build_env.py /etc/init.d/git-http-server /etc/init.d/git-build-dispatcher; chmod 0644 $CT_DIR/app.py; chmod 0750 $CT_DIR/configure_build_env.py; chmod 0755 /etc/init.d/git-http-server /etc/init.d/git-build-dispatcher /home/git/hooks/post-receive /home/git/hooks/mirror-sync.sh /home/git/hooks/build_image.py /home/git/hooks/build_submit.py /home/git/hooks/build_dispatcher.py; for repo in /home/git/repos/*.git; do [ -d \"\$repo\" ] || continue; mkdir -p \"\$repo/hooks\"; ln -sfn /home/git/hooks/post-receive \"\$repo/hooks/post-receive\"; done'" 2>&1
ssh -o StrictHostKeyChecking=accept-new -o ConnectTimeout=10 "$PROX_TARGET" \
