...
RUNTIME_ENV_NAMES="GIT_HTTP_HOST GIT_HTTP_PORT GIT_SSH_HOST GIT_DEFAULT_BRANCH REGISTRY_ADDRESS REGISTRY_USER REGISTRY_PASSWORD REGISTRY_INSECURE BUILDKIT_ADDRESS BUILDX_BUILDER GIT_HOOK_LOGS_ROOT GIT_HOOK_LOG_RETENTION_DAYS"
...
echo "--- 2. Allow inherited environment through OpenRC ---"
ssh -o StrictHostKeyChecking=accept-new -o ConnectTimeout=10 "$PROX_TARGET" \
  "pct exec $GIT_CONTAINER_ID -- sh -c 'set -e; marker=\"# git-server runtime environment\"; expected=\"rc_env_allow=\\\"\\\${rc_env_allow} $RUNTIME_ENV_NAMES\\\"\"; if ! grep -Fqx \"\$expected\" /etc/rc.conf; then stamp=\$(date -u +%Y%m%dT%H%M%SZ); cp -p /etc/rc.conf /etc/rc.conf.bak-\$stamp; printf \"\\n%s\\n%s\\n\" \"\$marker\" \"\$expected\" >> /etc/rc.conf; fi'" 2>&1
...
echo "--- 4. Stage application, service, and hooks ---"
ssh -o StrictHostKeyChecking=accept-new -o ConnectTimeout=10 "$PROX_TARGET" \
...
ssh -o StrictHostKeyChecking=accept-new -o ConnectTimeout=10 "$PROX_TARGET" \
  "pct exec $GIT_CONTAINER_ID -- rc-service sshd restart" 2>&1
ssh -o StrictHostKeyChecking=accept-new -o ConnectTimeout=10 "$PROX_TARGET" \
  "pct exec $GIT_CONTAINER_ID -- rc-service git-http-server restart" 2>&1

echo "--- 7. Health check ---"
ssh -o StrictHostKeyChecking=accept-new -o ConnectTimeout=10 "$PROX_TARGET" \
  "pct exec $GIT_CONTAINER_ID -- sh -c 'healthy=; attempt=1; while [ \"\$attempt\" -le 15 ]; do if wget -qO- http://127.0.0.1:$GIT_HTTP_PORT/ >/dev/null; then healthy=1; break; fi; sleep 1; attempt=\$((attempt + 1)); done; [ \"\$healthy\" = 1 ] || { rc-service git-http-server status >&2; tail -50 $CT_DIR/server.log >&2; exit 1; }; rc-service git-http-server status'" 2>&1
